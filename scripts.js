...
google.load("visualization", "1", {
			...
			function drawChart() {
				...
				var myPopularSeries = [];//an empty array to put all the data

				var myObs = AllMyData.tags;//changed the name tag to follow the D.R.Y principle
				...
				for (var i = 0; i < myObs.length; i++) {
					...
					namesAndPopulartiy.push(myObs[i].popularity);//the string is already in digit, so I don't need the Number function to convert.
				...
				data.addColumn('string', 'Name');//add the hAxis
				data.addColumn('number', 'Popularity');//add the vAxis
